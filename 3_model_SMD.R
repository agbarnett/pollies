# 3_model_SMD.R
# Model the standardised mortality difference (observed minus expected)
# Can use both weighted and unweighted data
# March 2020
#source('R/3_make_Bayes_SMD.R') # makes the WinBUGS models
...
bdata = list(N = N, diff = SMR$SMD, X = X, J = J) # 
inits = list(beta=rep(0, J), tau=1) # initial values 
inits = rep(list(inits), n.chains) # repeat per chains

# run BUGS
bfile = 'bayes.ns.difference.txt' # from 3_make_Bayes.R
parms = c('beta','regress','tau')
...
for (i in 1:nrow(SMR)){
  SMR$fitted[i] = mean(to.estimate[i, ])
  SMR$lower[i] = quantile(to.estimate[i, ], probs=0.025)
  SMR$upper[i] = quantile(to.estimate[i, ], probs=0.975)
}
# plot SMR with spline predictions over time
ns.plot = ggplot(data=SMR, aes(x=year, y=SMD)) + 
  geom_hline(yintercept = 0, lty=0, col='dark red')+ # reference line at SMR = 1
...
if(weighted==FALSE){outfile = paste('figures/nonlineardf', df, '/SMD', country, '.nonlinear.', df ,'df.jpg', sep='')}
if(weighted==TRUE){outfile = paste('figures/nonlineardf', df, '/SMD', country, '.nonlinear.', df ,'df.weighted.jpg', sep='')}
...
bfile = 'bayes.ns.difference.txt' # can use same winbugs file as previous model
...
if(weighted==TRUE){outfile = paste('results/', country, '.DIC.weighted.SMD.RData', sep='')}
if(weighted==FALSE){outfile = paste('results/', country, '.DIC.SMD.RData', sep='')}
...
linear.plot = ggplot(data=SMR, aes(x=year, y=SMD)) + 
  geom_hline(yintercept = 0, lty=1, col='dark red')+ # reference line at SMR = 1
  geom_line(size=0.2, col=grey(0.4))+
  geom_point(col=grey(0.4), size=0.9)+
  geom_line(aes(x=year, y=fitted), col='blue', size=1.05)+
  geom_line(aes(x=year, y=lower), col='blue', lty=2, size=1.05)+
...
if(conference == TRUE){
  ...
  linear.plot = linear.plot +
    geom_label(data=labels, size=3.5, aes(x=year, y=SMD, label=label, col=factor(colour)), hjust=0, show.legend = FALSE)+
...
if(weighted==FALSE){outfile = paste('figures/linear/SMD', country, '.linear.jpg', sep='')}
if(weighted==TRUE){outfile = paste('figures/linear/SMD', country, '.linear.weighted.jpg', sep='')}
...
# save the estimates
if(weighted==FALSE){outfile = paste('results/Results', country, '.SMD.RData', sep='')}
if(weighted==TRUE){outfile = paste('results/Results', country, '.SMD.weighted.RData', sep='')}
