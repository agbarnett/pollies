...
for (country in countries){
  ...
  # b) SMR/SMD
  if(weighted==FALSE){infile = paste('results/Results', country, '.', stype, '.RData', sep='')} # from 3_model_SMR.R or 3_model_SMD.R
  if(weighted==TRUE){infile = paste('results/Results', country, '.', stype, '.weighted.RData', sep='')} # from 3_model_SMR.R or 3_model_SMD.R
  load(infile)
...
stats.extremes = group_by(to.plot, country) %>% # stats on min/max 
  ...
  summarise(min = roundz(min(fitted),2), # round with trailing zeros
            max = roundz(max(fitted),2))
...
stats = full_join(stats.extremes, stats.latest, by='country') %>%
  mutate(
    ...
    upper = 0, 
    label = paste(max, '\n', min, '\n', roundz(fitted,2), sep=''), # without labels
    #label = paste("Max = ", max, '\nMin=', min, '\nLatest=', round(SMR,2), sep=''), # with labels
    year = 1816) # add (x,y) coordinates
...
mplot = ggplot(data=to.plot.plus.legend, aes(x=year, y=SMR)) + 
  ...
  xlab('Year')+
  theme_bw()+
  theme(text = element_text(size=10), # reduce overall text size
        strip.text.x = element_text(size = 6, margin = margin(t=0.5,r=0, b=0.5, l=0, "mm")), # reduce size of facet and text size
        axis.text.x = element_text(size=6))+ # reduce text size
...
if(stype=='SMR'){mplot = mplot + 
  ...
  # legend:
  geom_label(data=stats, aes(x=year, y=SMR, label=label), size=1.5, hjust=0, vjust=1, label.padding = unit(0.1, "lines"))+ # alignment for top-left corner; reduce white space around numbers
  ...
  geom_label(data=legend, aes(x=1816, y=2, label='Max\nMin\nLatest'), hjust=0, vjust=1, size=1.5, label.padding = unit(0.1, "lines")) # labels
}
if(stype=='SMD'){mplot = mplot + 
  ...
  geom_text(data=legend, aes(x=1919, y=-30, label='Politicians\n living longer'), size=2.5)+
  geom_label(data=legend, aes(x=1816, y=20, label='Max\nMin\nLatest'), hjust=0, vjust=1, size=2.5) # labels
}
...
tplot
# export
outfile = paste('figures/MultiCountryMean.', stype, '.jpg', sep='')
jpeg(outfile, width=5.5, height=3.3, units='in', res=1200, quality=100)
print(tplot)
dev.off()
...
if(stype=='SMR'){mplot1 = mplot1 +
  ...
  geom_text(data=legend, aes(x=1919, y=1.8, label='Politicians\nliving shorter'), size=2)+
  geom_text(data=legend, aes(x=1919, y=1.03, label='Politicians equal\n to general population'), col='dark red', size=2)+
  geom_text(data=legend, aes(x=1919, y=0.3, label='Politicians\n living longer'), size=2)+
  geom_label(data=legend, aes(x=1816, y=2, label='Max\nMin\nLatest'), hjust=0, vjust=1, size=1.5) # labels
}
